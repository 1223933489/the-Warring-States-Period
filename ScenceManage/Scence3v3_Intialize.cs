...
//		kindOfPlayer = Enum_Role.Ahri;        //此行测试用 外部传入数据
	...
	void Start () {
		...
		road = new List<int>{ 1, 3 };
		for (int i = 1; i < 3; i++) {
			role_Players [i].GetComponent <Role_Main > ().roadLine = roadChoice ();
			role_AIs [i].GetComponent <Role_Main > ().roadLine = role_Players [i].GetComponent <Role_Main > ().roadLine;
		}
		role_AIs [0].GetComponent <Role_Main > ().roadLine = role_Players [0].GetComponent <Role_Main > ().roadLine = 2;
	...
	public int roadChoice()
	{
		int i = Random.Range (0, road.Count);
		int temp = road [i];
		road.RemoveAt (i);
		Debug.Log ("temp:" + temp);
		return temp;
	...
	public void IntializeOtherkill(string pathTemp){
		...
		for (int i = 0; i < 3; i++) {
			...
			if (i != 0) {
				role_Players [i].AddComponent<HeroAi> ();
			}
			...
			role_AIs [i].AddComponent<HeroAi> ();
		}
	}
