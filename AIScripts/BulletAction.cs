...
public class BulletAction : MonoBehaviour {
	...
	public IEnumerator Akt(){
		...
		float time = 0;
		while (target !=null && Vector3 .Distance (transform .position ,target .position+Vector3.up*1.3f)>0.3f) {
			time += Time.deltaTime;
			transform.position = Vector3.Lerp (startPos, target.position+Vector3.up*1.3f, time * moveSpeed);
		...
		if (target!=null ) {
			if (roleMain != null) {
				...
				target.GetComponent <RoleInfo> ().GetHurt (attack_Physical, attack_Magic,Type_Allrole.soldier);	
			}
			Destroy (this.gameObject);
			yield return null;
		}
