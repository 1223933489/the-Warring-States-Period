...
public abstract  class Role_Main : RoleInfo {   //与徐宝骏合并	
	...
	public void ChangMesh(bool change){
		Renderer[] mesh2 = GetComponentsInChildren<Renderer> ();
		for (int i = 0; i < mesh2 .Length; i++) {
			mesh2 [i].enabled = change;
		}
	}
	...
	public void SetKillForKey(){  //使用键盘调用技能函数；ui也需要调用相应的函数
		...
		if (Input .GetKeyDown (KeyCode .Q)) {
			if (skill_Q .timeCount >=skill_Q .CD && skill_Q .level>0 && Mp >=skill_Q .Mp && !IsDeath  ) {
				transform.rotation = Quaternion.LookRotation (skillLookDir);
				SetSkill_Q ();
				skill_Q.timeCount = 0f;
				Mp -= skill_Q.Mp;
				uimanager.op.StartCoroutine ("CDCount","Q");
			}

		}
		if (Input .GetKeyDown (KeyCode .W )) {
			if (skill_W .timeCount >=skill_W .CD && skill_W .level>0 && Mp >=skill_W .Mp && !IsDeath) {
				transform.rotation = Quaternion.LookRotation (skillLookDir);
				SetSkill_W ();
				skill_W.timeCount = 0f;
				Mp -= skill_W.Mp;
				uimanager.op.StartCoroutine ("CDCount","W");
			}
		}
		if (Input .GetKeyDown (KeyCode .E )) {
			if (skill_E .timeCount >=skill_E .CD && skill_E .level>0 && Mp >=skill_E .Mp && !IsDeath) {
				transform.rotation = Quaternion.LookRotation (skillLookDir);
				SetSkill_E ();
				skill_E.timeCount = 0f;
				Mp -= skill_E.Mp;
				uimanager.op.StartCoroutine ("CDCount","E");
			}
		}
	}
	public void SetSkillForUI(SkillButtonName  skillName){   //为委托准备的借口
		...
		case SkillButtonName.Button_E :
			if (skill_E .timeCount >=skill_E .CD && skill_E .level>0 && Mp >=skill_Q .Mp && !IsDeath) {
				transform.rotation = Quaternion.LookRotation (skillLookDir);
				SetSkill_E ();
				skill_E.timeCount = 0f;
				Mp -= skill_E.Mp;
	...
	public void SetOtherSkill_Weak(){
		if (target !=null ) {
			StartCoroutine ("SetWeak", "3");
		}
	}
	IEnumerator SetWeak(float count){
		float temp = 0;
		while (temp <= count) {
			temp++;
			target.GetComponent <Role_Main> ().moveSpeed -= 2.3f;//需要后期修改参数,并且限制时间
			yield return null ;
		}
		yield break;
	...
	public void SetOtherSkill_Quicken(){
		if (target !=null ) {
			StartCoroutine ("SetQuick", "3");
		}                            //需要后期修改参数
	}
	IEnumerator SetQuick(float count){
		float temp = 0;
		while (temp <= count) {
			temp++;
			target.GetComponent <Role_Main> ().moveSpeed += 1.5f;//需要后期修改参数,并且限制时间
			yield return null ;
		}
		yield break;
	}
